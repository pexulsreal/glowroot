...
public class Crystal1111Feature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public Crystal1111Feature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(GlowrootModBlocks.GLOWROOT_LEAVES_FALLEN.get(), GlowrootModBlocks.GLOWROOTED_STONE.get());
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("glowroot", "crystalnew1"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (!base_blocks.contains(context.level().getBlockState(new BlockPos(i, j, k)).getBlock()))
					continue;
				BlockPos spawnTo = new BlockPos(i + -1, j + -3, k + -1);
